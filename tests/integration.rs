...
use bxdb::{IndexMode, ReadDb, WriteDb};
...
fn open_read(dir: &Path) -> ReadDb {
    ReadDb::open(dir).expect("open read")
}
...
fn write_read_single_snapshot_btree() {
    ...
    let mut wdb = WriteDb::open(&dir, 4, 0).unwrap();
...
fn zero_pages_and_reads_without_blob_io() {
    ...
    let mut wdb = WriteDb::open(&dir, 2, 0).unwrap();
...
fn delta_chain_floor_query_multiple_snapshots() {
    ...
    let mut wdb = WriteDb::open(&dir, 2, 256).unwrap();
    ...
    wdb.save_pages(&memory, &bitmap, n_pages, 20).unwrap();
    ...
    wdb.save_pages(&memory, &bitmap, n_pages, 40).unwrap();
    drop(wdb);
...
fn threshold_forces_new_full_and_reads_stay_correct() {
    ...
    let mut wdb = WriteDb::open(&dir, 1, 4).unwrap();
    wdb.save_pages(&memory, &bitmap, n_pages, 1).unwrap();
    ...
    wdb.save_pages(&memory, &bitmap, n_pages, 2).unwrap();
    ...
    wdb.save_pages(&memory, &bitmap, n_pages, 3).unwrap();
    drop(wdb);
...
fn load_all_pages_mixed_kinds() {
    ...
    let mut wdb = WriteDb::open(&dir, 3, 128).unwrap();
    ...
    wdb.save_pages(&memory, &bitmap2, n_pages, 200).unwrap();
    drop(wdb);
...
fn bxdb_convert_binary_runs() {
    ...
    let mut wdb = WriteDb::open(&dir, 1, 256).unwrap();
    wdb.save_pages(&memory, &bitmap, n_pages, 1).unwrap();
    drop(wdb);
