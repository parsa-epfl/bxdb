...
pub(crate) struct LoadTiming {
    ...
    pub floor_jumps: Counter, // binary-search iterations in floor_mmap
...
extern "C" fn on_exit_report() {
    ...
    let floor_jumps = LOAD_TIMING.floor_jumps.get();
    ...
    let _ = write!(
        j,
        "{{\"pid\":{pid},\"total_calls\":{total_calls},\"total_ns\":{total_ns},"
    );
    ...
    let _ = write!(
        j,
        "\"floor\":{{\"ns\":{floor_ns},\"calls\":{total_calls},\"jumps\":{floor_jumps}}},"
    );
    ...
    let _ = write!(
        j,
        "\"cache_get_ns\":{fc_get_ns},\"cache_get_calls\":{total_calls},"
    );
    let _ = write!(j, "\"cache_hits\":{fc_hits},");
    let _ = write!(
        j,
        "\"decompress_ns\":{fc_decomp_ns},\"decompress_calls\":{fc_decomp_calls},"
    );
    let _ = write!(
        j,
        "\"cache_put_ns\":{fc_put_ns},\"cache_put_calls\":{fc_decomp_calls}}},"
    );
    ...
    let _ = write!(
        j,
        "\"base_cache_get_ns\":{dc_base_get_ns},\"base_cache_get_calls\":{delta_calls},"
    );
    let _ = write!(
        j,
        "\"base_lookup_ns\":{dc_base_lookup_ns},\"base_lookup_calls\":{delta_calls},"
    );
    let _ = write!(
        j,
        "\"base_decompress_ns\":{dc_base_decomp_ns},\"base_decompress_calls\":{delta_calls},"
    );
    let _ = write!(
        j,
        "\"base_cache_put_ns\":{dc_base_put_ns},\"base_cache_put_calls\":{delta_calls},"
    );
    let _ = write!(
        j,
        "\"blob_read_ns\":{dc_blob_ns},\"blob_read_calls\":{delta_calls},"
    );
    let _ = write!(
        j,
        "\"patch_ns\":{dc_patch_ns},\"patch_calls\":{delta_calls}}},"
    );
    ...
    let _ = write!(
        j,
        "\"io\":{{\"blob_read_ns\":{blob_read_ns},\"blob_read_calls\":{blob_read_calls},"
    );
    let _ = write!(
        j,
...
pub(crate) fn init_timing() {
    if !REGISTERED.swap(true, Ordering::SeqCst) {
        unsafe {
            libc::atexit(on_exit_report as extern "C" fn());
        }
    }
}
